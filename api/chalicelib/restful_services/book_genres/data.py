...
def get_book_genres_by_book_id(book_id: Union[str, uuid4]) -> list:
    ...
    """
    book_genres = db.SESSION.query(FctBookGenres).filter_by(fct_book_id=book_id).all()
    return PopulatedBookGenreSchema(many=True).dump(book_genres) if book_genres else []


def get_book_genres_by_genre_id(genre_id: Union[str, uuid4]) -> list:
    ...
    """
    book_genres = db.SESSION.query(FctBookGenres).filter_by(fct_genre_id=genre_id).all()
    return PopulatedBookGenreSchema(many=True).dump(book_genres) if book_genres else []
...
def get_book_genre_by_id(book_genre_id: Union[str, uuid4]) -> Optional[dict]:
    ...
    """
    book_genre = db.SESSION.query(FctBookGenres).filter_by(id=book_genre_id).one_or_none()
    return PopulatedBookGenreSchema().dump(book_genre) if book_genre else None
